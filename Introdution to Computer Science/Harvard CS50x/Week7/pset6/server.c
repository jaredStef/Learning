...
char* indexes(const char* path)
{
	...
	pathCopy = (strcpy(pathCopy, path));
	
	if (pathCopy[strlen(pathCopy) - 1] != '/')
	{
		pathCopy = malloc(strlen(path) + 2);
		pathCopy = (strcpy(pathCopy, path));
		pathCopy[strlen(pathCopy)] = '/';
	}
	
	char* currentIndex = malloc(strlen(path) + 1 + 9);
	strcpy(currentIndex, pathCopy);
	strcat(currentIndex, "index.php");
	
	for(int i = 0; i < 2; i++)
	{
		FILE* file = fopen(currentIndex, "r");
		...
		currentIndex = malloc(strlen(path) + 1 + 10);
		currentIndex = strcpy(currentIndex, pathCopy);
		strcat(currentIndex, "index.html");
	}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // before I had this set to a char and it was weird because the favicon image would load, but the happy cat would not
    // which made me sad, bc I wanted to see that happy cat, so I changed it to an int and now it works
    // probably has to do with a value being beyond the range of a char aka short 
    // computers are amazing. 
    int c; 
    
    // the file in memory
    char* fileData = malloc(1); 
    
    // the byte currently accessed
    int currentSize = 0; 
    ...
    while(true)
    {
        ...
        // add to data
        fileData[currentSize] = c;
        // grow data by 1
        currentSize += 1; 
        
        fileData = realloc(fileData, currentSize + 1); 
    }
    
    // if size is more than 1 which it was origionally allocated to then it loaded something
    bool success = sizeof(fileData) > 1;
    if (success)
    {
        *content = fileData; 
        *length = currentSize; 
...
bool pathEndsWith(const char* path, char* str)
{
    char* startOfExtension = strrchr(path, '.');
    return strcmp(startOfExtension, str) == 0; 
}
...
const char* lookup(const char* path)
{
    char* fileTypes[] = {".css", ".html", ".gif", ".ico", ".jpg", ".js", ".php", ".png"};
    char* fileResponces[] = {"text/css", "text/html", "image/gif", "image/x-icon", "image/jpeg",
    "text/javascript", "text/x-php", "image/png"}; 
    
    for (int i = 0; i < 8; i++)
    {
        if (pathEndsWith(path, fileTypes[i]))
        {
            return fileResponces[i];
        }
    }
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
	...
	if (method == NULL || requestTarget == NULL || httpVersion == NULL) { error(400); return false; }
	
	if (strcmp(method, "GET") != 0) { error(405); return false; }
	
	if (strcmp(httpVersion, "HTTP/1.1") != 0) { error(505); return false; }
	
	if (strchr(requestTarget, '"') != NULL || spaceCount > 2) { error(400); return false; }
	
	strtok(requestTarget, "?");
	
	char* possibleQuery = strtok(NULL, "?");
	
	if (requestTarget[0] != '/') { error(501); return false; }
	
	strcpy(query, possibleQuery == NULL ? "\0" : possibleQuery);
